...
| **Password Cryptography** | BCrypt.Net-Next | 4.0.3 | Industrial salted, non-reversible BCrypt password hashing |
| **API Documentation** | Swagger / OpenAPI (Swashbuckle) | 6.5.0 | Interactive API documentation & endpoint testing UI with JWT Bearer input |
...
- **Security Utility**: `PasswordHasher` (`BCrypt.Net-Next` Work Factor 11)
- **Key Responsibilities**:
  - Validates staff credentials via secure `POST /api/Logins/login` receiving `LoginDto` (`Username`, `Password`).
  - Implements **transparent auto-migration**: legacy plain text passwords in `LoginRegistration` are automatically hashed with BCrypt upon first successful login and saved back to the database.
  - Determines associated `Role`, `Staff`, and `Doctor` identity.
  - Generates signed **JWT tokens** with rich claims (`NameIdentifier`, `Name`, `Role`, `RoleId`, `DoctorId`, `StaffId`, `Jti`).
  - Enforces `[Authorize]` attributes across controllers (`Doctors`, `Staffs`, `Receptions`, `Pharmacists`, `Labs`).
  - Swagger UI equipped with JWT Bearer authorization input.
